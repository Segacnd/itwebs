...
const ConsultModal = ({ setOpen }) => {
    let groupState = useAppSelector(state => state.mainReducer.requestGroup.group);
    ...
    const sendConsultRequests = (e) => {
        ...
        e.preventDefault();
        if(!allow){
            return toast.error('Вы не приняли соглашение на обработку персональных данных')
        }
    ...
    return (
        <>
            {loading && <Loader />}
            <div ref={component} className={styles.modalComponent} onClick={(e) => { close(); e.stopPropagation() }}>
                <div ref={container} className={styles.modalContainer} onClick={(e) => e.stopPropagation()}>
                    <div className={styles.modalCross} onClick={close}>
                    ...
                    <form onSubmit={sendConsultRequests} className={styles.modalItems}>
                        ...
                        <div className={styles.inputPair}>
                            <label>Имя</label>
                            <input style={{ color: errorItem === 'name' ? 'red' : '', border: errorItem === 'name' ? '1px solid red' : '' }} type='text' value={inputObj.name} onChange={(e) => {
                                setInputObj(prev => (
                        ...
                        <div className={styles.inputPair}>
                            ...
                            <input type='text' style={{ color: errorItem === 'email' ? 'red' : '', border: errorItem === 'email' ? '1px solid red' : '' }} value={inputObj.email} onChange={(e) => {
                                setInputObj(prev => (
                        ...
                        <div className={styles.inputPair}>
                            ...
                            <input type='text' value={inputObj.phone} onChange={(e) => setInputObj(prev=>({
                                ...prev,
                                phone: '+' + e.target.value.split('').filter(item=>/[0-9]/.test(item)).join('')
                            }))} />
                        </div>
                        <div className={styles.inputPair}>
                            <label>Ваше сообщение</label>
                            <textarea type='text' value={inputObj.comment} onChange={(e) => setInputObj(prev => (
                                { ...prev, comment: e.target.value }
                            ))} />
                        </div>
                        <div className={styles.policyBlock}>
                            <div className={styles.policyInputContainer} >
                                <input type='checkbox' value={allow} onChange={() => setAllow(prev => !prev)} />
                            </div>
