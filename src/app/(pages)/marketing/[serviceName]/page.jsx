...
export const loadServiceInfo = async (name) => {
    const defaultData = {
        name: 'Интернет-маркетинг для всех сфер деятельности',
    ...
    try {
        console.log(name)
        const response = await axios.get(`${process.env.BACKSEC}://${process.env.BACKHOST}:${process.env.BACKPORT}/app/marketing/${name}`, {
            timeout: 5000,
            headers: {
...
export const generateMetadata = async ({ params }) => {
    ...
    const data = await loadServiceMeta(serviceName);
    const titles = {
    ...
    return {
        title: data && data.title ? data.title : titles[serviceName] ? titles[serviceName] : 'Интернет-маркетинг на территории Беларуси',
        description: data && data.description ? data.description : descs[serviceName] ? descs[serviceName] : 'Мы оказываем самые популярные виды услуг в области интернет-маркетинга на территории Беларуси'
    }
}
    ...
    return (
        <>
            <div className={styles.detailHeading}>
                <BackArrow />
                <h1>{data.name ? data.name : 'Интернет-маркетинг для всех сфер деятельности'}</h1>
            </div>
            <div className={styles.headBanner}>
                ...
                <motion.div
                    ...
                    <Image className={styles.detailImage} fill={true} src={`${process.env.BACKSEC}://${process.env.BACKHOST}:${process.env.BACKPORT}/perfomance/${data.detailImageName||'default.jpg'}`} alt={data.name} title={data.name} />
                </motion.div>
            </div>
            ...
            <ServiceSeoBlock link={serviceName} ways={data.ways} dops={data.dops} />
        </>
    )
}
