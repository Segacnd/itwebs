...
export const loadServiceInfo = async (name) => {
    ...
    try {
        const response = await axios.get(`${process.env.BACKSEC}://${process.env.BACKHOST}:${process.env.BACKPORT}/app/perfomance/${name}`, {
            timeout: 5000,
            headers: {
...
export const generateMetadata = async ({ params }) => {
    ...
    let serviceName = pars.perfomanceName;
    ...
    return {
        title: data && data.title ? data.title : titles[serviceName] ? titles[serviceName] : 'Perfomance Marketing | ITWEBS',
        description: data && data.description ? data.description : descs[serviceName] ? descs[serviceName]  : 'Perfomance Marketing | ITWEBS'
    }
}

const DetailService = async ({ params }) => {
    const pars = await params;
    const serviceName = pars.perfomanceName;
    const data = await loadServiceInfo(serviceName);
    return (
        <>
            <div className={styles.detailHeading}>
                <BackArrow />
                <h1>{data?.name ? data.name : 'Привлечение клиентов из сторонних ресурсов'}</h1>
            </div>
            ...
            <ServiceSeoBlock link={'perfomance-'+serviceName} ways={data?.ways} dops={data?.dops} />
        </>
    )
}
